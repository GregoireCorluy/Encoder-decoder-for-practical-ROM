...
weights = np.load("data-files/extraData/kernelDensity/2026-08-06-kernelDensity-Malik-CFLF-2022-condition-CO2-dataCenteredScaled.npy")
...
PCsource = dataSourceMajor @ PCA_components
...
np.save(f"data-files/costs/variance_kernelDensityPCA_{scaling}_{species_selection_name}-bw_{start_bw}_{end_bw}_{nbr_points_bw}-dataset_{dataset_type}{'-sample-norm-var' if sample_norm_var else ''}{'-sample-norm-range' if sample_norm_range else ''}.npy", variance_data)
...
costs = cost_function_normalized_variance_derivative(variance_data,
                                                    ...
                                                    norm=None)
np.save(f"data-files/costs/costs_kernelDensityPCA_{scaling}_{species_selection_name}-bw_{start_bw}_{end_bw}_{nbr_points_bw}-p_{power}-ver_sh_{vertical_shift}-dataset_{dataset_type}{'-sample-norm-var' if sample_norm_var else ''}{'-sample-norm-range' if sample_norm_range else ''}.npy", costs)
...
plt.savefig(f"data-files/costs/plot_Dhat_kernelDensityPCA_{scaling}_{species_selection_name}-bw_{start_bw}_{end_bw}_{nbr_points_bw}-p_{power}-ver_sh_{vertical_shift}-dataset_{dataset_type}{'-sample-norm-var' if sample_norm_var else ''}{'-sample-norm-range' if sample_norm_range else ''}.png")
#plt.show()
